  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-zinc-800">
        ...
        <span className="text-xs font-semibold px-3 py-1 rounded-full bg-indigo-950/60 text-indigo-300 border border-indigo-800/60 self-start sm:self-auto">
        ...
        {upcoming.map((m, idx) => (
          <div
            ...
            className={`p-4 rounded-xl border transition-all flex flex-col justify-between ${
              idx === 0
                ? 'bg-gradient-to-br from-indigo-950/40 via-zinc-900 to-slate-900 border-indigo-500/50 shadow-md ring-1 ring-indigo-500/30'
          ...
          >
            <div>
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                {idx === 0 && (
                  <span className="text-[10px] uppercase font-bold text-indigo-300 bg-indigo-950/80 border border-indigo-800/60 px-2 py-0.5 rounded-full">
                    Next Major
                  </span>
                )}
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-zinc-800 flex items-center justify-between text-xs font-semibold text-indigo-400">
      ...
      {past.length > 0 && (
        <div className="mt-8 pt-6 border-t border-zinc-800">
          <button
            ...
            className="w-full flex items-center justify-between p-3.5 rounded-xl bg-zinc-900/60 border border-zinc-800 text-sm font-bold text-zinc-200 hover:bg-zinc-800/80 transition-colors"
          >
          ...
          {showPast && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-4 animate-fadeIn">
              {past.map((m) => (
                <div
                  key={m.age}
                  className="p-3.5 rounded-xl bg-[#09090b] border border-zinc-800 flex items-start justify-between gap-2 opacity-80 hover:opacity-100 transition-opacity"
                >
                  <div>
                    <span className="text-xs font-bold text-emerald-400 flex items-center gap-1">
                      <CheckCircle2 className="w-3.5 h-3.5" /> Age {m.age} Milestone
                    </span>
                    <span className="block text-sm font-semibold text-zinc-200 mt-0.5">
                      {formatMediumDate(m.date)}
                    </span>
                    <span className="text-[11px] text-zinc-500">
                      {m.dayOfWeek}
