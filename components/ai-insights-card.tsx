    ...
    <div className="bg-white dark:bg-[#18181b] rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-2xl border border-slate-200/90 dark:border-zinc-800/90 p-5 sm:p-8 max-w-4xl mx-auto mt-8 transition-all">
      ...
      {loading && (
        <div className="py-8 flex flex-col items-center justify-center text-center">
          ...
          <span className="text-xs text-slate-500 dark:text-zinc-500 mt-1">Powered by server-side Gemini intelligence</span>
        </div>
      )}

      {error && (
        <div className="mt-4 p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-xs text-amber-800 dark:text-amber-200 flex items-start gap-2">
      ...
      {insight && (
        <div className="mt-6 p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-violet-50/80 via-slate-50 to-indigo-50/50 dark:from-violet-950/30 dark:via-zinc-900 dark:to-slate-900 border border-violet-200 dark:border-violet-800/60 animate-fadeIn">
          <div className="flex items-center justify-between gap-2 mb-3">
            ...
            <button
              ...
              className="text-xs font-medium text-slate-500 hover:text-indigo-600 dark:text-zinc-400 dark:hover:text-indigo-400 flex items-center gap-1"
            >
      ...
      {!insight && !loading && (
        <div className="mt-4 text-center py-6 px-4 rounded-xl border border-dashed border-slate-300 dark:border-zinc-800 bg-slate-50 dark:bg-[#09090b]">
          <p className="text-xs text-slate-600 dark:text-zinc-400">
            Click <strong className="text-slate-900 dark:text-zinc-200">Generate Era Insights</strong> above to explore cultural trends, major world moments, and milestone wisdom for someone born in {birthParts.year}.
          </p>
        </div>
      )}
    </div>
