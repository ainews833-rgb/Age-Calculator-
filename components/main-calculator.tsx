...
export function MainCalculator({
  ...
  return (
    <div
      ...
      className="bg-[#18181b] rounded-2xl shadow-2xl border border-zinc-800/90 p-5 sm:p-8 max-w-4xl mx-auto transition-all"
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-6 border-b border-zinc-800">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-[11px] font-medium text-zinc-500">Presets:</span>
          <button
            type="button"
            onClick={() => handlePresetSample(2000, 1, 15)}
            className="text-xs px-2.5 py-1 rounded-md bg-zinc-900 text-zinc-300 hover:bg-zinc-800 hover:text-white border border-zinc-800 transition-colors"
          >
            Jan 15, 2000
          </button>
          <button
      ...
      {errorMsg && (
        <div
          role="alert"
          className="mt-5 p-3.5 bg-rose-950/40 border border-rose-850 rounded-xl text-rose-300 text-sm flex items-start gap-2.5 animate-fadeIn"
        >
          <AlertCircle className="w-5 h-5 flex-shrink-0 mt-0.5 text-rose-400" />
          <div>
            <p className="font-semibold text-rose-200">Calculation Notice</p>
            <p className="text-xs mt-0.5 text-rose-300/90">{errorMsg}</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        {/* Date of Birth Block */}
        <div className="bg-zinc-900/60 p-4 sm:p-5 rounded-xl border border-zinc-800">
          <div className="flex items-center justify-between mb-3">
            <label className="block text-sm font-bold text-white">
              1. Date of Birth <span className="text-rose-400">*</span>
            </label>
            ...
            <input
              ...
              className="text-xs px-2 py-1 bg-[#09090b] border border-zinc-800 rounded-md text-zinc-200 cursor-pointer focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            {/* Month */}
            <div>
              <label htmlFor="dob-month" className="block text-xs font-semibold text-zinc-400 mb-1">
                Month
              </label>
              <select
                id="dob-month"
                ...
                className="w-full h-11 px-2.5 sm:px-3 bg-[#09090b] border border-zinc-800 rounded-lg text-sm font-medium text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 shadow-xs"
              >
            ...
            {/* Day */}
            <div>
              <label htmlFor="dob-day" className="block text-xs font-semibold text-zinc-400 mb-1">
                Day
              </label>
              <select
                id="dob-day"
            ...
            {/* Year */}
            <div>
              <label htmlFor="dob-year" className="block text-xs font-semibold text-zinc-400 mb-1">
                Year
              </label>
              <select
                id="dob-year"
          ...
          <div className="mt-3.5 pt-3 border-t border-zinc-800/80">
            ...
            {includeTime && (
              <div className="mt-2 grid grid-cols-2 gap-2 animate-fadeIn">
                <div>
                  <label htmlFor="dob-hour" className="block text-[11px] text-zinc-400 mb-0.5">
                    Hour (0-23)
                  </label>
                  <input
                    id="dob-hour"
                    ...
                    className="w-full h-9 px-3 bg-[#09090b] border border-zinc-800 rounded-lg text-xs text-white focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
                <div>
                  <label htmlFor="dob-min" className="block text-[11px] text-zinc-400 mb-0.5">
                    Minute (0-59)
                  </label>
                  <input
                    id="dob-min"
        ...
        <div className="bg-zinc-900/60 p-4 sm:p-5 rounded-xl border border-zinc-800">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="text-xs px-2.5 py-0.5 bg-indigo-950/80 text-indigo-300 hover:bg-indigo-900 rounded font-medium border border-indigo-800 transition-colors"
              >
                Today
              </button>
              <input
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            {/* Target Month */}
            <div>
              <label htmlFor="target-month" className="block text-xs font-semibold text-zinc-400 mb-1">
                Month
              </label>
              <select
                id="target-month"
            ...
            {/* Target Day */}
            <div>
              <label htmlFor="target-day" className="block text-xs font-semibold text-zinc-400 mb-1">
                Day
              </label>
              <select
                id="target-day"
            ...
            {/* Target Year */}
            <div>
              <label htmlFor="target-year" className="block text-xs font-semibold text-zinc-400 mb-1">
                Year
              </label>
              <select
                id="target-year"
          ...
          {bMonth === 2 && bDay === 29 && (
            <div className="mt-3.5 pt-3 border-t border-amber-900/60 bg-amber-950/30 p-2.5 rounded-lg">
              <span className="block text-[11px] font-semibold text-amber-300 mb-1">
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-4 border-t border-zinc-800">
        <button
          type="button"
          onClick={handleReset}
          className="order-2 sm:order-1 h-12 px-5 rounded-xl border border-zinc-700 bg-zinc-900 text-zinc-300 font-semibold text-sm hover:bg-zinc-800 hover:text-white focus:ring-2 focus:ring-zinc-600 transition-colors flex items-center justify-center gap-2"
        >
